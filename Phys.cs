...
namespace Fall_Project_2._0
{
    public partial class Phys : Form
    {
        ...
        void refresh()
        {
            ...
            MySqlDataAdapter da = new MySqlDataAdapter("select * from phys", fall);
        ...
        {
            MySqlConnection fall = new MySqlConnection("Server = localhost; Database = journal of progress; uid=root; pwd= ; Charset=utf8");
            MySqlDataAdapter da = new MySqlDataAdapter("insert into phys(cname,cevaluation,cdate) values('" + FullName.Text + "','" + Evaluationn.Text + "','" + Date.Text + "')", fall);
            DataTable dt = new DataTable();
            da.Fill(dt);
            fall.Close();
            refresh();
        ...
        {
            MySqlConnection fall = new MySqlConnection("Server = localhost; Database = journal of progress; uid=root; pwd= ; Charset=utf8");
            MySqlDataAdapter da = new MySqlDataAdapter("delete from phys where id_c=" + Dell.Text, fall);
            DataTable dt = new DataTable();
            da.Fill(dt);
            fall.Close();
            refresh();
